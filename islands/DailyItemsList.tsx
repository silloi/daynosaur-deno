...
import { type DairyItem } from "@/utils/db.ts";
...
export interface DailyItemsListProps {
...
export default function DailyItemsList(props: DailyItemsListProps) {
  ...
  console.log("itemsSig.value.", itemsSig.value)
  ...
  return (
    <div>
      {itemsSig.value.length
        ? itemsSig.value.map((item) => {
          return (
            <p key={item.id}>
              <a href={`/diary/${item.date}`}>{item.date}</a>
            </p>
          );
        })
        : <EmptyItemsList />}
