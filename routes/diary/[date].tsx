...
export default defineRoute<State>(async (_req, ctx) => {
  ...
  const dailyItem = await getItemByUserAndDate(ctx.state.sessionUser.login, date);
  console.log(new Date().toISOString());
  console.log("dailyItem", dailyItem);

  return (
    <>
      <Head title={date} href={ctx.url.href}>
      ...
      <main class="flex-1 flex flex-col justify-center mx-auto w-full space-y-16 p-4 max-w-6xl">
        <div>
          ...
          <p class="text-gray-500 text-right">{date}</p>
        </div>
          ...
          <div class="flex-1">
            <div
              ...
              dangerouslySetInnerHTML={{
                __html: render(dailyItem?.content ?? ""),
              }}
            />
          </div>
        </div>
